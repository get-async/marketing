...
it('strips fragments from URL', function (): void {
    $url = 'http://async.test/docs/concepts/hierarchical-structure#key-principles';

    $marketingPage = new CreateOrRetrieveMarketingPage($url)->execute();

    expect($marketingPage->url)->toBe('http://async.test/docs/concepts/hierarchical-structure');
});

it('strips query parameters from URL', function (): void {
    $url = 'http://async.test/docs/concepts/hierarchical-structure?param1=value1&param2=value2';
...
it('strips both fragments and query parameters from URL', function (): void {
    $url = 'http://async.test/docs/concepts/hierarchical-structure?param1=value1#key-principles';
...
it('handles URLs with ports correctly', function (): void {
    $url = 'http://async.test:8080/docs/concepts/hierarchical-structure?param1=value1#key-principles';

    $marketingPage = new CreateOrRetrieveMarketingPage($url)->execute();

    expect($marketingPage->url)->toBe('http://async.test:8080/docs/concepts/hierarchical-structure');
});

it('handles URLs without paths correctly', function (): void {
    $url = 'http://async.test?param1=value1#key-principles';

    $marketingPage = new CreateOrRetrieveMarketingPage($url)->execute();

    expect($marketingPage->url)->toBe('http://async.test');
});

it('handles URLs without query parameters or fragments', function (): void {
    $url = 'http://async.test/docs/concepts/hierarchical-structure';
...
it('increments pageviews when creating or retrieving marketing page', function (): void {
    ...
    $url2 = 'http://async.test/docs/concepts/hierarchical-structure';
